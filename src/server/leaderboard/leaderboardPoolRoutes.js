...
import { getHistoricalSnapshotCandidates } from "../snapshots/snapshotReader.js";

function mapPoolPlayer(player) {
  return {
...
export function createLeaderboardPoolRouter({
  fetchCandidates = fetchRankCandidates,
  getSnapshotCandidates = getHistoricalSnapshotCandidates
} = {}) {
  ...
  router.get("/api/leaderboard/pool", async (request, response) => {
    try {
      ...
      if (historical) {
        const historicalResult = await getSnapshotCandidates({ leaderboardScope, rankMin, rankMax });
        if (historicalResult.status !== "ready") {
          return response.status(404).json({
            ...
            snapshot: historicalResult.snapshot || null
          });
        }
        candidates = historicalResult.players;
        snapshot = historicalResult.snapshot;
      } else {
        candidates = await fetchCandidates(leaderboardScope, rankMax);
      }
      ...
      response.json({
        ...
        source: historical ? "historical-snapshot" : "upstream",
        snapshot
      });
    } catch (error) {
...
export default createLeaderboardPoolRouter();
